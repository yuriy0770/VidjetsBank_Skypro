...
def wrapper(f):
    ...
    def inner(*args):
        with open(r"report\report.txt", "w", encoding="utf-8") as file:
...
def spending_by_category(
    ...
    """Траты по категории"""
    try:
        logger.info("Используются данные из файла operations.xlsx строк.")
        ...
        result_json = {"category": category, "total_spending": total_spending}
...
def spending_by_weekday(transactions: pd.DataFrame, date: Optional[str] = None) -> dict:
    ...
    try:
        logger1.info("Используются данные из файла operations.xlsx строк.")
        ...
        weekday_spending = (
            filtered_transactions.groupby(
                filtered_transactions["Дата операций"].dt.dayofweek
            )["Сумма платежа"]
            .sum()
            .reset_index()
        )
        ...
        weekday_spending["День недели"] = weekday_names[
            weekday_spending["Дата операций"].dt.dayofweek
        ]
        ...
        result_json = [
            {
                ...
                "Средняя сумма платежа": (
                    spending / 3 if i == len(weekday_spending) - 1 else spending
                ),
            }
            for i, (day, spending) in enumerate(
                zip(weekday_spending["День недели"], weekday_spending["Сумма платежа"])
            )
        ]
